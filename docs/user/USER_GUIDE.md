...
- The global search base is shown as a bounded table of source entities. Keywords and exclusions are also bounded so long lists do not stretch the screen.
- Intent signals use the same simplified source/rule model as qualification criteria. A global `0 / 1 / 2` scoring rubric is defined once, and individual signals can override it only when needed.
...
- Sources are structured entities with type, label, reference, and trust policy: trusted, cross-check, or HITL required.
- Intent signals have detection rules and a global fixed `0/1/2` scoring rubric. Fit and intent scoring use presets; tier thresholds classify the result.
