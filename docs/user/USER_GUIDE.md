...
The `ICP Radar` screen now starts with a list-first catalog of configured radars. Each row represents a configured ICP Radar with aligned columns for status, owner, cadence, last run, run mode, candidate count, needs-review count, and accepted count. Use `Create radar` to add a browser-local demo radar, or open `ТОиР / SIBUR` to inspect the active fixture-backed radar. Its `Shortlist` tab contains the current ranked table, and its `Settings` tab edits the radar definition by block: overview, global search base, account qualification rules, monitoring, intent signals, scoring model, and validation.
...
- Account qualification criteria are written as a name, natural-language rule description, requirement level, and source policy.
- Rule and signal source policies select shared sources by name, can add local sources, can use the global search base, and can allow additional system-selected sources.
- Intent signals have a name, description, detection rules in natural language, local/shared sources, and a fixed `0 / 1 / 2` rubric table.
...
- `Settings` is edited by block. Each block has its own `Edit`, `Save`, and `Discard`, and saved edits are stored only in browser local demo state and labelled as `Local draft`.
...
- Qualification rules define which legal entities should enter the radar. Intent signals define why a qualified account is interesting now. These are separate settings blocks.
- Sources are structured entities with type, label, reference, and trust level. Rules and signals reference them through source policies instead of repeating free-text URLs.
- Intent signals have detection rules and a fixed `0/1/2` scoring rubric. Fit and intent scoring use presets; tier thresholds classify the result.
