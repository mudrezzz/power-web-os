...
- a backend-backed global `Playbook` screen for product, semantic-role, access-rule and version configuration;
...
`Playbook` is global and does not depend on the selected account. Account-specific
route explanation is available in `Access Plans` under `Rule analysis`. Its
segmented control switches between the generated current playbook analysis and
the deterministic `No partner motion` variant. The frontend does not
recalculate planner logic; both previews are pre-generated in the Access Plan
artifact.
...
### Products and Playbook

Open `Playbook` to configure what the team sells and which semantic buying
functions matter for that product. The first screen is a product catalog. Open
a product to use its full-width Product, Buying roles and Versions tabs. Changes
are explicit: save the draft or publish a new immutable version. Published
versions can be opened read-only, activated or restored into a new draft.

Buying roles are business functions such as technical reliability owner or
economic sponsor. Open a role row to edit it directly below the row. The basic
form asks only for role name, responsibility, whether it is required and its
organizational scope. Optional decision rights, priority override and exclusion
guidance are under Advanced. Roles are not job titles or people. Concrete
account titles and search wording will be proposed later by Power Web Discovery
without changing this policy.

Access rules are not part of people-discovery configuration. Old published
...
rules shape a concrete plan. The view shows allowed routes, blocked channels,
available assets, review rules, policy decisions and route preview. The
`No partner motion` variant is generated by the Python planner and lets the
user compare what changes when partner intro motions are disabled.
