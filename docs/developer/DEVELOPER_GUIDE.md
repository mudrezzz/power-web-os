...
- Settings are edited by block: Overview, Global search base, Account qualification rules, Monitoring, Intent signals, Scoring model, and Validation. Do not reintroduce one global edit mode.
- Sources are entities, not textarea blobs. The UI selects shared sources by name, can add local per-rule/per-signal sources, and stores generated source ids only as internal contract fields.
...
- The localStorage overlay shape is keyed by `radar_id` and stores `{ override_type, radar, saved_at }`. `override_type` is `created` or `edited`; UI-only radar statuses may be `local_draft` or `modified_locally`.
