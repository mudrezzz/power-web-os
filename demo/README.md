...
- Backend-backed SmartDiagnostics and Industrial Energy Optimization product configuration with semantic buying roles and immutable versions.
- Versioned Radar-product bindings and deterministic Power Web handoff fixtures:
  the benchmark Radar prepares 14 product-scoped role demands (8 + 6), with no
  provider call or persisted people-search run.
- A remote-only live acceptance CLI can take the eight-role SmartDiagnostics
  subset handoff through title hypotheses, 24 official/HH/generic search tasks,
  receipts, source leads and coverage checkpoints. This optional stage spends
  OpenRouter tokens and does not populate the UI or create `power-web-run-*`.
- Playbook analysis for the selected account under Access Plans, including allowed routes, blocked channels, assets, review rules, and route policy decisions.
...
The seeded `Benchmark / SIBUR holding contour` Radar contains one accepted and
one review-needed evidence-complete handoff candidate. Use the candidate Power
Web tab to prepare an immutable brief. The review-needed candidate requires
acknowledgement. This demo action creates no Radar, Signal Monitoring or Power
Web discovery run.
...
## Product And Playbook Demo

The API seed creates the active `SmartDiagnostics` product with eight semantic
buying roles. Open `Playbook` in the frontend, choose the product from the
catalog, then inspect or edit the full-width backend draft, publish immutable
versions and view version history. Role rows expand inline and expose four basic
business fields; optional guidance stays collapsed. Account-specific Playbook
Analysis remains under `Access Plans` -> `Rule analysis`.

New publications do not create AccessPlaybook versions. Existing access-route
snapshots remain readable only inside historical-version detail and continue to
support the independent legacy Access Planner compatibility contour.

Validate the complete UI/API/DB contour without search or provider calls:
...
The check tests both languages at 1280x720 and 1366x768, measures full workspace
usage and inline-editor width, and fails on API, console, overlap or body-scroll
errors. It does not invoke search providers or create Radar/Power Web runs.
