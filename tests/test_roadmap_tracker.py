...
def test_sqlite_repository_crud_and_ordering(tmp_path: Path) -> None:
    ...
    repository.update_track("0.7.6.4.1", "power-web")
    assert repository.get_slice("0.7.6.4.1").track == "power-web"  # type: ignore[union-attr]
...
def test_cli_import_list_show_render_and_check(tmp_path: Path) -> None:
    ...
    run_cli("update-track", "0.7.6.4.1", "power-web")
    assert "0.7.6.4.1" in run_cli("list", "--track", "power-web").stdout
