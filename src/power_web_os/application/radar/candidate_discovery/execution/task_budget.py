...
class RadarExecutionBudgetSettings:
    """Candidate-discovery task budget settings for one staged run.

    Owns:
    - Hard task limits and semantic task reserve limits for candidate-discovery execution.

    Does not own:
    - Provider-level external-call accounting or signal-monitoring budget policy.

    Architecture:
    docs/architecture/radar/CANDIDATE_DISCOVERY_EXECUTION_ARCHITECTURE.md#radarexecutionbudgetsettings
    """
...
class RadarBudgetDecision:
    """Candidate-discovery task budget admission decision.

    Owns:
    - Accepted/rejected task admission state, budget key, limit, current count, and reserve metadata.

    Does not own:
    - Provider-call counters, checkpoint decisions, or final artifact projection.

    Architecture:
    docs/architecture/radar/CANDIDATE_DISCOVERY_EXECUTION_ARCHITECTURE.md#radarbudgetdecision
    """
...
class RadarExecutionBudget:
    """Count candidate-discovery provider tasks with Radar semantic keys.

    Owns:
    - Candidate-discovery task admission, semantic task reserve counters, warnings, and exhaustion events.

    Does not own:
    - External-call accounting, useful-result retry assessment, or signal-monitoring budgets.

    Architecture:
    docs/architecture/radar/CANDIDATE_DISCOVERY_EXECUTION_ARCHITECTURE.md#radarexecutionbudget
    """
...
class SubjectTaskBudget(RadarExecutionBudget):
    ...
    Owns:
    - Compatibility construction of candidate-discovery task budgets from a single subject limit.

    Does not own:
    - New task-budget policy, external budgets, or signal-monitoring budgets.

    Architecture:
    docs/architecture/radar/CANDIDATE_DISCOVERY_EXECUTION_ARCHITECTURE.md#subjecttaskbudget
    """
