...
EVALUATION_ARTIFACT_VERSION = "0.7.6.4"
...
def evaluate_radar_dossier(
    *,
    ...
    public_surface_quality = _public_surface_quality(visible_rows)
    benchmark_context = _benchmark_context(run=run, dossier=dossier)
    closeout_summary = _blind_benchmark_closeout_summary(
        benchmark_context=benchmark_context,
        benchmark_target_funnel=benchmark_target_funnel,
        metrics={
            "strict_recall": _ratio(len(strict_hits), len(legal_baseline)),
            "visible_recall": _ratio(len(legal_visible_hits), len(legal_baseline)),
            "accepted_product_candidate_count": accepted_product_count,
            "review_needed_candidate_count": review_needed_count,
            "duplicate_candidate_id_count": public_surface_quality["duplicate_candidate_id_count"],
            "empty_provenance_candidate_count": public_surface_quality["empty_provenance_candidate_count"],
        },
    ...
    report = {
        ...
        "benchmark_context": benchmark_context,
        "blind_benchmark_closeout": closeout_summary,
        "metrics": {
            "strict_recall": _ratio(len(strict_hits), len(legal_baseline)),
        ...
        "diagnostics": {
            ...
            **upstream_counts,
            ...
            "benchmark_target_closeout_reasons": _count_by(
                str(item.get("closeout_path_reason") or "") for item in closeout_summary["baseline_target_results"]
            ),
...
def _benchmark_context(*, run: dict[str, Any], dossier: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(hints_used, bool):
        if isinstance(task_context.get("benchmark_hints_used"), bool):
...
def _blind_benchmark_closeout_summary(
    *,
    ...
    target_results = [
        {
            **item,
            "closeout_path_reason": _closeout_path_reason(item),
        }
        for item in benchmark_target_funnel
    ]
    missing_targets = [item for item in target_results if not item.get("projected")]
    top_miss_reasons = _count_by(
        str(item.get("closeout_path_reason") or "unknown")
        for item in missing_targets
    )
    strict_recall = metrics.get("strict_recall")
    requires_followup = strict_recall == 0 or any(
        str(item.get("closeout_path_reason") or "") in {"unknown", ""}
        for item in missing_targets
    )
    return {
        ...
        "requires_followup_rca": requires_followup,
...
def _closeout_path_reason(item: dict[str, Any]) -> str:
    ...
    if reason in {
        ...
        "completion_cap_exhausted",
        "completion_lane_quota_exhausted",
        "selector_priority_lost",
        "completion_not_selected",
        "expansion_not_selected",
    }:
        return "not_selected"
    if reason in {
        "source_found_not_projected",
    ...
    if reason in {
        "expansion_searched_no_support",
        "expansion_searched_not_projected",
    }:
        return "source_not_found"
    if reason in {
        "expansion_not_executed",
        "expansion_global_budget_limited",
        "external_budget_limited",
        "expansion_budget_limited",
        "expansion_reserve_limited",
        "scheduler_rejected",
    }:
        return "not_executed"
...
def _public_surface_quality(rows: list[dict[str, Any]]) -> dict[str, int]:
    seen: set[str] = set()
...
def _has_public_provenance(item: dict[str, Any]) -> bool:
    ...
    if any(str(item.get(key) or "").strip() for key in reason_keys):
        return True
    return bool(item.get("review_flags"))
